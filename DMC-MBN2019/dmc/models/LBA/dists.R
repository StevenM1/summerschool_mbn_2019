...
make.r.vGF <- function (drifts, b, A, n_v, t0, st0 = 0, n, return.ttf = FALSE) 
{
    drifts[drifts < 0] <- 0
    starts <- matrix(runif(min = 0, max = A, n = n * n_v), nrow = n_v)
    ttf <- t0 + (b - starts)/drifts
...
rlba.norm.vGF <- function (n, A, b, t0, mean_v, sd_v, return.ttf = FALSE) 
{
    if (any(b < A)) 
        stop("b cannot be smaller than A!")
    n_v <- ifelse(is.null(dim(mean_v)), length(mean_v), dim(mean_v)[1])
    drifts <- matrix(rnorm(n = n * n_v, mean = mean_v, sd = sd_v), 
        nrow = n_v)
    make.r.vGF(drifts = drifts, b = b, A = A, n_v = n_v, t0 = t0, 
        st0 = 0, n = n, return.ttf = return.ttf)
}



n1PDFvGF <- function (rt, A, b, t0, mean_v, sd_v, st0 = 0, silent = FALSE) {
  ...
  p_omission <- prod(pnorm(0,mean_v,sd_v))
