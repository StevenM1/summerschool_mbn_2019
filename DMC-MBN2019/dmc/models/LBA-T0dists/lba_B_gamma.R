...
#     stop("Trasform has not created parameter(s) required by the model.")
  ...
  par.df[,c("A","b","t0","mean_v","sd_v","shape","scale")]
}

random.dmc<- function(n,p.df,model)
{
  rlba.norm.gamma(n,A=p.df$A,b=p.df$b,t0=p.df$t0,
...
likelihood.dmc <- function(p.vector,data,ok.types=c("normgamma"),min.like=1e-10) 
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      switch(attr(attributes(data)$model,"type"),
        normgamma=n1PDF.norm.gamma(data$RT[attr(data,"cell.index")[[i]]],
          A=p.df$A,
          b=p.df$b,
          t0=p.df$t0[1], 
          mean_v=p.df$mean_v,
