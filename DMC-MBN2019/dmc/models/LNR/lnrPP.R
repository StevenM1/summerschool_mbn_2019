# n-choice LNR, t0, mu, sigma parameterization, sigma and t0 on log scale
#    External parameters types: meanlog, log(sdlog), log(t0), st0 (optional)
...
# Returns vector of likelihoods for each RT in data (in same order)
{
  ...
  {
    p.df <- p.df.dmc(p.vector,i,attr(data,"model"),n1order=TRUE)
